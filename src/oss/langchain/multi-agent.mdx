...
<Columns cols={3}>
  <Card title="Context management" icon="brain">
    Surface relevant knowledge without overwhelming the context window. Different tasks need different context.
  </Card>
  ...
  <Card title="Parallelization" icon="arrows-split-up-and-left">
...
Background jobs keep the main agent responsive during long-running tasks. When work takes minutes—reviewing contracts, conducting research, auditing code—the main agent kicks off a subagent as a background job and continues conversing with the user while the work completes.
...
The conversational agent handles memory and context; the router stays stateless. This avoids the complexity of managing conversation history across multiple parallel agents.

**Alternative**: Full persistence
