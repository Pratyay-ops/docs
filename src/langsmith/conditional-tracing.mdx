...
When you have `LANGSMITH_TRACING=true` set globally to enable automatic tracing, you may still need to disable or customize tracing selectively for specific requests.

The @[`tracing_context`][tracing_context] context manager (Python) and @[`tracingEnabled`][tracingEnabled] option (TypeScript) allow you to override global tracing settings at runtime based on business logic, without restructuring your code or changing environment variables.
...
There is granular control with three priority levels:
