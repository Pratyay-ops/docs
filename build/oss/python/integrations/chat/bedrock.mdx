...
By default, the Bedrock Converse API buffers tool call arguments until the JSON is complete before releasing them. This means when using [tool calling](/oss/python/langchain/tools) or [structured output](/oss/python/langchain/structured-output), tokens appear to arrive "all at once" rather than streaming incrementally.
