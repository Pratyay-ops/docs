...
Skills are a way to bundle capabilities together into larger actions and provide additional context beyond single tool descriptions.
...
Agent Builder supports custom models. You can override the default model for a specific agent by editing the agent's settings:
...
<Note>
  You can use models hosted on your own infrastructure if they are accessible through a public API endpoint. LangSmith cannot connect to models hosted on private networks, behind VPNs, or on machines that are not exposed to the internet.
</Note>
