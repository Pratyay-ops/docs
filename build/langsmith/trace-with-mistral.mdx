...
- Set `LANGSMITH_TRACING=true` and provid3 your LangSmith API key (`LANGSMITH_API_KEY`) activates automatic logging of traces.
- Specify a [`LANGSMITH_PROJECT`](/langsmith/log-traces-to-project) name to organize traces by project; if not set, traces go to the default project (named "default"). The `LANGSMITH_TRACING` flag must be true for any traces to be recorded.
